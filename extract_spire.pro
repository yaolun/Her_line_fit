pro extract_spire, indir=indir, outdir=outdir, plotdir=plotdir, filename=filename, test=test,fixed_width=fixed_width,slw=slw,ssw=ssw,$
	...
	for i = 0, n_elements(line_center)-1 do begin
		dl = 1.2*1e9*(line_center[i]*1e-4)^2/2.998d10*1e4
	...
	; Perform double Gaussian fit on 370 um CI & CO blended lines
...
;		endelse
		...
		for i = 0, n_elements(line_name)-1 do begin
			...
			fit_line, pixelname[j], line_name[i], wlb, fluxb, status, errmsg, cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_para, snr, /baseline, outdir=plotdir,no_plot=no_plot
			...
			;Fitting part
			;if line_name[i] eq 'CO11-10' then stop
			;stop
			if keyword_set(fixed_width) then begin
				if keyword_set(test) then fit_line, pixelname[j], line_name[i], wll, fluxx, status, errmsg, cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_para, snr, line, plot_base=plot_base,$
				...
				if not keyword_set(test) then fit_line, pixelname[j], line_name[i], wll, fluxx, status, errmsg, cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_para, snr, line, plot_base=plot_base,$
		...
		if keyword_set(double_gauss) then begin
			...
			numb = ceil(dlb/(wl_diff[where(wl ge line_center_dg[2*i])])[0])
				...
				fit_line, pixelname[j], line_name_dg[2*i]+'_'+line_name_dg[2*i+1], wlb, fluxb, status, errmsg, cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_para, snr, /baseline, outdir=plotdir,no_plot=no_plot
				;extract the wave and flux for plottng that is for better visualization of the fitting results.
				ind_plot = where(wl gt base_range[0]-5*dl and wl lt base_range[3]+5*dl)
				plot_wl = wl[ind_plot] & plot_flux = flux[ind_plot]
				plot_base = [[wlb],[fluxb]]
				...
				;Fitting part
				;Different fitting keyword for fixed width and test arguement
				fit_line,pixelname[j],line_name_dg[2*i]+'_'+line_name_dg[2*i+1],wll,fluxx,status,errmsg,cen_wl,sig_cen_wl,str,sig_str,fwhm,sig_fwhm,base_para,snr,line,/double_gauss,outdir=plotdir,$
					 noiselevel=3,base_range=base_range,plot_base=plot_base,/fix_dg,/spire
		...
		if keyword_set(global_noise) then begin
			...
			if keyword_set(continuum_sub) then begin
				...
				continuum_sub = continuum_sub*1d26*1d6*(wl*1d-6)^2/c*1d4
			...
			if keyword_set(flat) then begin
				...
				if keyword_set(fx) then printf, sed, format='(2(a16,2x))','Wave (um)','Flux (Jy)'
    			if keyword_set(brightness) then printf, sed, format='(2(a16,2x))','Wave (um)','I_nu (Jy/as2)'
				flat = flux*1d26*1d6*(wl*1d-6)^2/c*1d4 - continuum_sub
