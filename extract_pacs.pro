pro extract_pacs, indir=indir, filename=filename, outdir=outdir, plotdir=plotdir, pospath=pospath, noiselevel=noiselevel, test=test, ra=ra, dec=dec,$
    ...
    line_center_oh2o = [55.13237858,55.84107632,56.81776703,57.39510658,58.70051448,61.31781921,62.92978624,63.32514386,63.91602319,63.95696098,$
	...
	range_oh2o = [[55,55.4],[55.34,56.34],[56.31,57.31],[56.89,57.89],[58.2,59.2],[60.81,61.81],[62.42,63.3],[62.82,63.85],[63.4,63.95],[63.92,64.45],$
	...
	line_center_ph2o = [55.85969505,55.98479901,56.02823528,56.32639972,56.77240026,57.63798074,57.71080360,58.37823620,59.98862729,60.16364998,$
						...
						148.7941027,156.1979172,158.3155136,159.4892632,167.0391189,169.7430470,170.1434151,174.6112595,187.1154299]
	...
	range_ph2o = [[55.84,55.95],[55.86,56.02],[55.99,56.3],[56.03,56.7],[56.4,56.8],[57.4,57.7],[57.65,58.21],[57.87,58.65],[59.48,60.1],[60,60.2],$
				  ...
				  [148.75,149.29],[155.69,156.25],[157.9,158.81],[159.45,160],[166.9,167.5],[169.24,170.1],[169.8,170.64],[174.11,174.9],[186.61,187.7]]
	...
	cont_ph2o = [[50.76,70,70,90],[50.76,70,70,90],[50.76,70,70,90],[50.76,70,70,90],[50.76,70,70,90],[50.76,70,70,90],[50.76,70,70,90],[50.76,70,70,90],[50.76,70,70,90],[50.76,70,70,90],$
				 ...
				 [140,150,150,171],[140,150,150,171],[140,150,150,171],[140,150,150,171],[140,150,150,171],[140,150,150,171],[140,150,150,171],[174,180,180,190],[174,180,180,190]]
				 ;[90,99,99,101.45],[90,99,99,101.45],[90,99,99,101.45],[90,99,99,101.45],[90,99,99,101.45],[90,99,99,101.45]
...
;					'CO45-44','CO46-45','CO47-46','CO48-47']
	line_center_co = [65.687911815,67.338144584,69.076142800,70.909022876,72.844686840,74.891943997,77.060638546,79.361807645,81.807868093,84.412840643,$
					  ...
					  137.19978120,144.78783174,153.27056228,162.81572308,173.63579573,186.00397423];64.11905135,62.62586464,61.20295216,59.84552508,$
;					  58.54922029,57.31005417,56.12438218,54.98886358]
	range_co = [[65.19,66.19],[66.84,67.84],[68.57,69.57],[70.41,71.41],[72.34,73.34],[74.39,75.39],[76.56,77.56],[78.86,79.86],[81.31,82.31],[83.91,84.5],$
			    ...
			    [136.696,137.696],[144.28,145.28],[152.77,153.77],[162.31,163.31],[173.13,174.13],[184.499,186.499]]
	...
	cont_co = [[62,65.19,66.19,66.84],[66.19,66.84,67.84,68.57],[67.84,68.57,69.57,70.41],[69.57,70.41,71.41,72.34],[71.41,72.34,73.34,74.39],[73.34,74.39,75.39,76.56],[75.39,76.56,77.56,78.86],[77.56,78.86,79.86,81.31],[79.8,81.31,82.31,83.91],[80,83.91,84.5,89],$
			   ...
			   [132,137.1,137.35,139],[140,144,145,145.5],[152.5,153.1,153.4,157],[158,162.7,162.95,168],[168,173.5,173.75,174.0],[180,185.9,186.1,186.5]]
	
	line_name_oh = ['OH19-14','OH18-15','OH13-9','OH12-8','OH14-10','OH15-11','OH5-1','OH4-0','OH9-3','OH8-2',$
				    'OH10-8','OH11-9','OH3-1','OH2-0','OH14-12','OH15-13','OH19-16','OH7-5','OH6-4'];,'OH18-17' too close to CII line 
	line_center_oh = [55.89230701,55.95141064,65.13336752,65.28048087,71.17262181,71.21722533,79.11753556,79.18106327,84.42236779,84.59877137,$
					  115.1505737,115.3844118,119.2374049,119.4444962,134.8415187,135.9598052,154.7834879,163.0192292,163.4001337];,157.8098369
    ...
    if not keyword_set(current_pix) then begin
		printf, firstfit, format='(17(a16,2x))', $
    		'Line','LabWL(um)','ObsWL(um)','Sig_Cen(um)','Str(W/cm2)','Sig_str(W/cm2)','FWHM(um)','Sig_FWHM(um)','Base(W/cm2/um)','SNR','E_u(K)','A(s-1)','g','RA(deg)','Dec(deg)','Blend'
    endif else begin
        ...
        if status le 0 then begin
            ...
            if not keyword_set(current_pix) then begin
            	printf, firstfit, format = '((a16,2X),9(g16.10,2X),2(g16.10,2X),(i16,2x),2(g16.10,2X),(a16,2x))',$
            		line_name[i], line_center[i], cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_str,snr, E_u, A, g, ra, dec, blend_msg
            endif else begin
            	printf, firstfit, format = '((a16,2X),9(g16.10,2X),2(g16.10,2X),(i16,2x),2(g16.10,2X),2(a16,2x))',$
            		line_name[i], line_center[i], cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_str,snr, E_u, A, g, ra, dec, current_pix, blend_msg
            endelse
        endelse 
    endfor
    ; Double Gaussian fit
		...
		for i = 0, n_elements(line_center_dg)/2-1 do begin
			...
			if status eq 0 then begin
				...
				if not keyword_set(current_pix) then begin
					...
					printf, firstfit, format = '((a16,2X),9(g16.10,2X),2(g16.10,2X),(i16,2x),2(g16.10,2X),(a16,2x))',$
						line_name_dg[2*i+1], line[3], cen_wl[1], sig_cen_wl[1], str[1], sig_str[1], fwhm[1], sig_fwhm[1], base_str[1],snr[1], E_u[1], A[1], g[1], ra, dec, blend_msg
				endif else begin
					printf, firstfit, format = '((a16,2X),9(g16.10,2X),2(g16.10,2X),(i16,2x),2(g16.10,2X),2(a16,2x))',$
						line_name_dg[2*i], line[0], cen_wl[0], sig_cen_wl[0], str[0], sig_str[0], fwhm[0], sig_fwhm[0], base_str[0],snr[0], E_u[0], A[0], g[0], ra, dec, current_pix, blend_msg
					printf, firstfit, format = '((a16,2X),9(g16.10,2X),2(g16.10,2X),(i16,2x),2(g16.10,2X),2(a16,2x))',$
						line_name_dg[2*i+1], line[3], cen_wl[1], sig_cen_wl[1], str[1], sig_str[1], fwhm[1], sig_fwhm[1], base_str[1],snr[1], E_u[1], A[1], g[1], ra, dec, current_pix, blend_msg
				endelse
			endelse
		endfor
	endif
    ...
    if not keyword_set(current_pix) then begin
    	...
    	printf, firstfit, format='(18(a16,2x))', $
    		'Line','LabWL(um)','ObsWL(um)','Sig_Cen(um)','Str(W/cm2)','Sig_str(W/cm2)','FWHM(um)','Sig_FWHM(um)','Base(W/cm2/um)','SNR','E_u(K)','A(s-1)','g','RA(deg)','Dec(deg)','Pixel_No.','Blend','Validity'
    endelse
    for line = 0, n_elements(line_name_n)-1 do begin
    	lowest = '0'
