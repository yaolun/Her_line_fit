pro extract_pacs, indir=indir, filename=filename, outdir=outdir, plotdir=plotdir, pospath=pospath, noiselevel=noiselevel, test=test, ra=ra, dec=dec,$
	...
	line_name_oh = ['OH19-14','OH18-15','OH13-9','OH12-8','OH14-10','OH15-11','OH5-1','OH4-0','OH9-3','OH8-2',$
				    'OH10-8','OH11-9','OH3-1','OH2-0','OH14-12','OH15-13','OH19-16','OH6-4'];,'OH18-17' too close to CII line ; ,'OH7-5' we think the wavelength is wrong from LAMDA
	line_center_oh = [55.89230701,55.95141064,65.13336752,65.28048087,71.17262181,71.21722533,79.11753556,79.18106327,84.42236779,84.59877137,$
					  115.1505737,115.3844118,119.2374049,119.4444962,134.8415187,135.9598052,154.7834879,163.4001337];,157.8098369; ,163.0192292
	range_oh = [[55.85,55.9],[55.9,56],[64.5,65.2],[65.2,66],[70.7,71.2],[71.2,72],[78.5,79.15],[79.15,80.5],[83.35,84.5],[84.5,85],$
				[114.5,115.3],[115.3,116],[118.81,119.4],[119.4,120.02],[134,135.5],[135.5,136.5],[154,155.5],[157,158.5],[163.3,164.5]]; ,[162.5,163.3]
	...
	line_name_other = ['OI3P1-3P2','NII_122','OI3P0-3P1','CII2P3_2-2P1_2','OH_hf_163.12']; ,'OH_hf_61.4'
	line_center_other = [63.1836709,121.9,145.48055764,157.69228158,163.12467358]; ,61.46556395
	range_other = [[62.73,63.73],[121.5,122.5],[145.13,146.13],[157.35,158.35],[163.07,163.17]]; ,[61.35,61.45]
	cont_other = [[62.9,63.1,63.3,63.6],[119,121.8,122.2,123],[144.95,145.45,145.65,152],[155,157.5,158.0,162],[162,163.07,163.17,165]]; ,[60,61.35,61.45,63]
	...
	line_name_dg = [['CO31-30','OH9-3'],['CO23-22','o-H2O4_14-3_03'],['p-H2O7_53-8_26','CO20-19'],['OH14-12','o-H2O5_14-5_05'],$
					...
					['o-H2O7_34-7_25','p-H2O6_24-6_15'],['p-H2O5_33-6_06','o-H2O3_03-2_12'],$; ,['CO16-15','OH7-5']  we think OH7-5 has the wrong wavelength
    ...
    if not keyword_set(current_pix) then begin
    	...
    	printf, firstfit, format='((a20,2x),18(a20,2x))', $
    ...
    for i = 0, n_elements(line_name)-1 do begin
        ...
        if status le 0 then begin
            printf, firstfit, format = '((a20,2X),(a50))', line_name[i], errmsg
        endif else begin
            ...
            ;
            if not keyword_set(current_pix) then begin
            	printf, firstfit, format = '((a20,2X),10(g20.10,2X),2(g20.10,2X),(i20,2x),2(g20.10,2X),(a20,2x))',$
	...
	if keyword_set(double_gauss) then begin
		for i = 0, n_elements(line_center_dg)/2-1 do begin
			...
			if status eq 0 then begin
				printf, firstfit, format = '((a20,2X),(a50))', line_name_dg[2*i]+'_'+line_name_dg[2*i+1], errmsg
			endif else begin
    ...
    if keyword_set(global_noise) then begin
		...
		if not keyword_set(current_pix) then begin
			printf, secondfit, format='((a20,2x),16(a20,2x))',$
				'Line','LabWL(um)','ObsWL(um)','Sig_Cen(um)','Str(W/cm2)','Sig_str(W/cm2)','FWHM(um)','Sig_FWHM(um)','Base(W/cm2/um)','Noise(W/cm2/um)','SNR','E_u(K)','A(s-1)','g','RA(deg)','Dec(deg)','Blend'
		endif else begin
    	...
    	for i = 0, n_elements(line_name)-1 do begin
        	...
        	if status le 0 then begin
				printf, secondfit, format = '((a20,2X),(a50))', line_name[i], errmsg
			endif else begin
            	; The read_line_ref procedure read the g, A, E_u from another file
            	read_line_ref, line_name[i], E_u, A, g
            	base_str = interpol(base, wll, cen_wl);*fwhm
				...
				if finite(snr,/nan) eq 1 then continue
				if not keyword_set(current_pix) then begin
					printf, secondfit, format = '((a20,2X),10(g20.10,2X),2(g20.10,2X),(i20,2x),2(g20.10,2X),(a20,2x))',$
            			line_name[i], line_center[i], cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_str, noise, snr, E_u, A, g, ra, dec, blend_msg
				endif else begin
		...
		if keyword_set(double_gauss) then begin
			for i = 0, n_elements(line_center_dg)/2-1 do begin
				...
				if status eq 0 then begin
					printf, secondfit, format = '((a20,2X),(a50))', line_name_dg[2*i]+'_'+line_name_dg[2*i+1], errmsg
				endif else begin
					; Making sure the line classification is correct
					i1 = where(line_name eq line_name_dg[2*i])
					i2 = where(line_name eq line_name_dg[2*i+1])
