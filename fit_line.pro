...
;		endif
		...
		if not keyword_set(no_plot) then begin
			...
			if keyword_set(single_gauss) then begin
				xyouts, 0.75, 0.7, 'SNR ='+string(snr,format='(g5.2)'), /normal
                xyouts, 0.75, 0.65, 'FWHM ='+string(fwhm,format='(g4.2)'), /normal
			endif
			if keyword_set(double_gauss) then begin
        		xyouts, 0.55, 0.7, 'SNR ='+string(snr[0],format='(g5.2)')+', FWHM ='+string(fwhm[0],format='(g4.2)'), /normal, color=30
				xyouts, 0.55, 0.65, 'SNR ='+string(snr[1],format='(g5.2)')+', FWHM ='+string(fwhm[1],format='(g4.2)'), /normal, color=225
