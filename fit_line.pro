...
;		endif
		...
		if not keyword_set(no_plot) then begin
			...
			if keyword_set(double_gauss) then begin
        		xyouts, 0.55, 0.75, 'SNR ='+string(snr[0],format='(g6.3)')+', FWHM ='+string(fwhm[0],format='(g6.3)'), /normal, color=30
				xyouts, 0.55, 0.7, 'SNR ='+string(snr[1],format='(g6.3)')+', FWHM ='+string(fwhm[1],format='(g6.3)'), /normal, color=225
